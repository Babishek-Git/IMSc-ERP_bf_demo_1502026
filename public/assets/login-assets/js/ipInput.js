(function($, window, document) {
  ...
  function IpInput(el, ip, init) { 
    ...
    this.el = el;
    //alert(JSON.stringify($(this).closest('td').attr('class'))); 
    if (init) {
      var elId = el.attr('data-id'); //alert(elId);
      this._init(elId);
    }
  }
    ...
    _init: function(elId) {
      this.el.html(
        '<div class="ip-input-container">' +
          '<input type="text" name="txt_'+elId+'_1[]" class="ip-input-item" value="' + (this.ipArr ? this.ipArr[0] : '') + '"/>' + 
          '<i class="ip-input-dot"></i>' + 
          '<input type="text" name="txt_'+elId+'_2[]" class="ip-input-item" value="' + (this.ipArr ? this.ipArr[1] : '') + '"/>' +
          '<i class="ip-input-dot"></i>' +
          '<input type="text" name="txt_'+elId+'_3[]" class="ip-input-item" value="' + (this.ipArr ? this.ipArr[2] : '') + '"/>' +
          '<i class="ip-input-dot"></i>' +
          '<input type="text" name="txt_'+elId+'_4[]" class="ip-input-item" value="' + (this.ipArr ? this.ipArr[3] : '') + '"/>' +
        '</div>'
      );
