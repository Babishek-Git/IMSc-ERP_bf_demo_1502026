										...
										@if(isset($data['AddressData']))
											@foreach($data['AddressData'] as $AddressData)
												...
												<tr data-name="{{ $AddressData->emp_no }}" data-status="{{ $AddressData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.empaddr-change-request',['id'=>encrypt($AddressData->change_req_id),'Page'=>encrypt('ALLREQ')]) }}'"> <i class='fa fa-edit'></i> View Details</button></td>
													<td align="center">
