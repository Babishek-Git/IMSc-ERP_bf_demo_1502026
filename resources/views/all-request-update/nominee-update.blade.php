					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Nominee Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl = 'change-request.nominee-update-request'; @endphp 
									@php $BackUrl = 'change-request.nominee-update-request'; @endphp 
									@if($NomineeCount>0)
									...
									<tbody id="rm-tableBody">
										@if(isset($data['NomineeData']))
											@foreach($data['NomineeData'] as $NomineeData)
												<tr data-name="{{ $NomineeData->emp_no }}" data-status="{{ $NomineeData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $NomineeData->emp_no }}</td>
													<td>{{ $NomineeData->emp_name_payslip }}</td>
													<td>{{ $NomineeData->designation_name }}</td>
													<td>NomineeData Update Request</td>
													<td>{{ $NomineeData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.nominee-update-request',['id'=>encrypt($NomineeData->change_req_id),'Page'=>encrypt('ALLREQ')]) }}'"> <i class='fa fa-edit'></i>View &  Edit </button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $NomineeData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
