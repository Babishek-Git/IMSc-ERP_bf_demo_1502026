										...
										@if(isset($data['IdData']))
											@foreach($data['IdData'] as $IdData)
												<tr data-name="{{ $IdData->emp_no }}" data-status="{{ $IdData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.id-card-request',['id'=>encrypt($IdData->change_req_id),'Page'=>encrypt('ALLREQ')]) }}'"> <i class='fa fa-edit'></i> View Details</button></td>
													<td align="center">
