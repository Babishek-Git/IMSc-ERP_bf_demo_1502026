					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Family Member Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl = 'change-request.empfamilydetails-change-request'; @endphp 
									@php $BackUrl = 'change-request.empfamilydetails-change-request'; @endphp 
									...
									<tbody id="rm-tableBody">
										@if(isset($data['FamilyData']))
											@foreach($data['FamilyData'] as $FamilyData)
												<tr data-name="{{ $FamilyData->emp_no }}" data-status="{{ $FamilyData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $FamilyData->emp_no }}</td>
													<td>{{ $FamilyData->emp_name_payslip }}</td>
													<td>{{ $FamilyData->designation_name }}</td>
													<td>Family Member Update Request</td>
													<td>{{ $FamilyData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.empfamilydetails-change-request',['id'=>encrypt($FamilyData->change_req_id), 'Page'=>encrypt('ALLREQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit </button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $FamilyData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
									</tbody>
