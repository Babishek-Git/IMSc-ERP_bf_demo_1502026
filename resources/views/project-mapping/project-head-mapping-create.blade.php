					...
					<blockquote class="bq1" style="overflow:auto">
						<div class="container">
							<div class="row">
								<div class="div12">
									<div class="form-box">
										<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Project Heads - Budget Heads Mapping </div></div></div>
											...
											<div class="divrowbox innerdiv pt-2">
                                                ...
                                                <div class="table-wrapper">
                                                    ...
                                                    <table class="attTable">
                                                        ...
                                                        <tbody> 
                                                            @if(isset($GiaMasterData))
                                                            @foreach($GiaMasterData as $ObjectHeadGroup)
                                                            <tr>
                                                                <td align="center">{{ $loop->iteration }}</td>
                                                                <td>{{ $ObjectHeadGroup->gia_name }}
																	<input type="hidden" id ='hidden_gia_id' name ='hidden_gia_id[]' value = '{{ $ObjectHeadGroup->gia_id }}'>
																</td>
                                                                <td width="50%">
																	 @php
																	 $GroupedData = $projectHeadData->groupBy('gia_id');
																		$savedLedgerIds = [];
																		if(isset($GroupedData) && isset($GroupedData[$ObjectHeadGroup->gia_id])){
																			$RawIds  = $GroupedData[$ObjectHeadGroup->gia_id]->pluck('object_head_grouop_id')->toArray();
																			if(filled($RawIds)){
																	...
																	@endphp
																	<select name="cmb_oh_id_[{{ $loop->index }}][]" id="cmb_oh_id_{{ $loop->index }}" class="tboxsmclass ChosenInput" multiple>
																		<option value="">---- Select ---</option>
																		@if(isset($ObjectHeadGroupData))
																			@foreach($ObjectHeadGroupData as $LedgerGroup)
																				<option value="{{ $LedgerGroup->object_head_group_id }}"
																					{{ in_array($LedgerGroup->object_head_group_id, $savedLedgerIds) ? 'selected' : '' }}>
																					{{ $LedgerGroup->full_heads }}
																				</option>
																			@endforeach
																		@endif
																	</select>
