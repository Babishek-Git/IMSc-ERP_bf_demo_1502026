					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Marital Status Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'change-request.marital-change-request'; @endphp 
									@php $BackUrl = 'change-request.marital-change-request-list'; @endphp 
									@if($Page == 'REQ_APPLY')
									@if($MaritalData>0)
									...
									<tbody id="rm-tableBody">
										@if(isset($data['MaritalData']))
											@foreach($data['MaritalData'] as $MaritalData)
												<tr data-name="{{ $MaritalData->emp_no }}" data-status="{{ $MaritalData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $MaritalData->ic_no }}</td>
													<td>{{ $MaritalData->emp_name_payslip }}</td>
													<td>{{ $MaritalData->designation_name }}</td>
													<td>Marital Status Update Request</td>
													<td>{{ $MaritalData->created_by }}</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('change-request.marital-change-request',['id'=>encrypt($MaritalData->change_req_id),'Page'=>encrypt('REQ_APPLY')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
														@endif
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('change-request.marital-change-request-process',['Page'=>encrypt($Page),'Application'=>encrypt($MaritalData->change_req_id),'action'=>encrypt('REQ_PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
													</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $MaritalData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
