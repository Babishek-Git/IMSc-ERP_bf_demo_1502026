...
if(isset($data['EditClaimData']))
{
	...
	$NewSpouseDob      = optional(json_decode($EditClaimData->new_value))->Spouse_dob ?? '';
	$NewSpouseName     = optional(json_decode($EditClaimData->new_value))->Spouse_name ?? '';
	$NewSpouseIncome   = optional(json_decode($EditClaimData->new_value))->Spouse_income ?? '';
	$NewSpouseAadhar   = optional(json_decode($EditClaimData->new_value))->Spouse_aadhar ?? '';
	$NewSpouseBloodGrp = optional(json_decode($EditClaimData->new_value))->Spouse_bloodgrp ?? '';

	$OldSpouseDob   = optional(json_decode($EditClaimData->old_value))->Spouse_dob ?? '';
	$OldSpouseName = optional(json_decode($EditClaimData->old_value))->Spouse_name ?? '';
				...
				<blockquote class="bq1" style="overflow:auto">
										...
										<div class="form-step active"> 
											<div class="row" align="right">
												...
												if($Page == 'REQ_APPLY'){
													...
													$BackUrl = 'change-request.marital-change-request-pending-list'; 
												}else{
													$BackUrl = 'change-request.marital-change-request-list'; 
												}
												@endphp
					...
					<div class="row">
						<div class="div12" align="center">
							...
							<input type="hidden" name="wf_module_code" id="wf_module_code" value="{{ encrypt('MARRIAGE') }}" />
