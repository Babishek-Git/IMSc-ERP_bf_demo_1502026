					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Bank Details Update Request List</span>
								...
								<div class="rm-toolbar-right">
                                    ...
                                    @php $AddUrl = 'change-request.bank-details-change-request'; @endphp 
									@php $BackUrl = 'change-request.bank-details-change-request-list'; @endphp 
									@if($Page == 'REQ_APPLY')
									@if($BankCount>0)
									...
									@endif
									@endif
									<!-- @php $AddUrl = 'change-request.empbankdetails-change-request'; @endphp 
									@php $BackUrl = 'request-updates.bank-details-update'; @endphp 
									@if($BankCount>0)
									    <button type="button" class="rm-new-emp-btn readonly">+ Edit Request</button>
									@else
										<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ Edit Request</button>
									@endif -->
								</div>
							</div>
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['BankData']))
											@foreach($data['BankData'] as $BankData)
											@php
												$ExistingAccountNo         = optional(json_decode($BankData->old_value))->account_no ?? '';
												$ExistingBankName          = optional(json_decode($BankData->old_value))->bank_name ?? '';
												$ExistingIfscCode          = optional(json_decode($BankData->old_value))->ifsc_code ?? '';
												$NewAccountNo              = optional(json_decode($BankData->new_value))->account_no ?? '';
												$NewBankName               = optional(json_decode($BankData->new_value))->bank_name ?? '';
												$NewIfscCode               = optional(json_decode($BankData->new_value))->ifsc_code ?? '';
											@endphp
												<tr data-name="{{ $BankData->emp_no }}" data-status="{{ $BankData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $BankData->ic_no }}</td>
													<td>{{ $BankData->emp_name_payslip }}</td>
													<td>{{ $BankData->designation_name }}</td>
													<td>Bank Update Request</td>
													<td>{{ $BankData->created_by }}</td>
                                                    <td align="center">
														@if($Page == 'REQ_APPLY')
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('change-request.bank-details-change-request',['id'=>encrypt($BankData->change_req_id),'Page'=>encrypt('REQ_APPLY')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
														@endif
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('change-request.bank-details-change-request-process',['Page'=>encrypt($Page),'Application'=>encrypt($BankData->change_req_id),'action'=>encrypt('REQ_PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
													</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
													...
													<!-- <td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.empbankdetails-change-request',['id'=>encrypt($BankData->change_req_id),'Page'=>encrypt('ALLREQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit </button></td>
													<td align="center">
														<button type="button" name="btn_delete" id="btn_delete" class="btn btn-default tdelbtn Delete" title="Click here to delete" style="cursor: pointer;"><i class="fa fa-trash-o pt2"></i></button>												
													</td> -->
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $BankData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
