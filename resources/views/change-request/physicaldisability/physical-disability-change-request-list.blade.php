...
@php
if(isset($data['PhysicalData']))
{
	$PhysicalData  = $data['PhysicalData'];
	$PhysicalCount = Count($PhysicalData);
}
$Page = $data['Page'] ?? NULL;
@endphp
<style>
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Physical Disability Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'change-request.physical-disability-change-request'; @endphp 
									@php $BackUrl = 'change-request.physical-disability-change-request-list'; @endphp 
									@if($Page == 'REQ_APPLY')
									@if($PhysicalCount>0)
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									<thead>
										...
										<th class="rm-sortable" data-col="name"><div class="rm-th-inner">Particulars<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:12px;height:12px"><path d="M7 8l5-5 5 5M7 16l5 5 5-5"/></svg></div></th>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['PhysicalData']))
											@foreach($data['PhysicalData'] as $PhysicalData)
												<tr data-name="{{ $PhysicalData->emp_no }}" data-status="{{ $PhysicalData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $PhysicalData->ic_no }}</td>
													<td>{{ $PhysicalData->emp_name_payslip }}</td>
													<td>{{ $PhysicalData->designation_name }}</td>
													<td>Physical Update Request</td>
													<td>{{ $PhysicalData->created_by }}</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('change-request.physical-disability-change-request',['id'=>encrypt($PhysicalData->change_req_id),'Page'=>encrypt('REQ_APPLY')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
														@endif
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('change-request.physical-disability-change-request-process',['Page'=>encrypt($Page),'Application'=>encrypt($PhysicalData->change_req_id),'action'=>encrypt('REQ_PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
													</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $PhysicalData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
