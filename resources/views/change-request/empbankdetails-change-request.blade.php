...
if(isset($data['EditCliamData']))
{
	...
	$ExistingBankName   = optional(json_decode($EditCliamData->old_value))->bank_name ?? '';
	$ExistingIfscCode   = optional(json_decode($EditCliamData->old_value))->ifsc_code ?? '';
	$ExistingAccNo      = optional(json_decode($EditCliamData->old_value))->account_no ?? '';
	$ExistingBranchAddr = optional(json_decode($EditCliamData->old_value))->branch_addr1 ?? '';
	$ExistingHolderName = optional(json_decode($EditCliamData->old_value))->account_holder_name ?? '';

	$NewBankName   = optional(json_decode($EditCliamData->new_value))->bank_name ?? '';
	$NewIfscCode   = optional(json_decode($EditCliamData->new_value))->ifsc_code ?? '';
	$NewAccNo      = optional(json_decode($EditCliamData->new_value))->account_no ?? '';
	$NewBranchAddr = optional(json_decode($EditCliamData->new_value))->branch_addr1 ?? '';
	$NewHolderName = optional(json_decode($EditCliamData->new_value))->account_holder_name ?? '';
