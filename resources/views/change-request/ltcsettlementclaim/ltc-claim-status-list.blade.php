					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">LTC Settlement Claim Status List</span>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['LTCData']))
											@foreach($data['LTCData'] as $LTCData)
												@if($LTCData->emp_no == session('WcmsEmpNo'))
													<tr data-name="{{ $LTCData->emp_no }}" data-status="{{ $LTCData->active == 1 ? 'active' : 'inactive' }}">
														...
														<td class="col" align="center">
															<button type="button" onclick="window.location='{{ route('ltc-claim.ltc-claim-status-list', ['id'=>encrypt($LTCData->ltc_advance_id),'modulecode'=>encrypt('LTCCLAIM')])}}'" class="btn btn-default tuploadbtn" title="Click here to View" style="cursor: pointer;"><i class="fa fa-hand-o-right"></i> View Detailed Status</button>
														</td>
													</tr>
												@endif
											@endforeach
