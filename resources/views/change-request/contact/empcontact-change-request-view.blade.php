...
@php
 ...
 if(isset($data['Empdata'])){
	...
	$ContactNo   = collect($Empdata)->pluck('emp_mobile')->first();
}
...
if(isset($data['EditClaimData']))
{
	...
	$ExistingContact = optional(json_decode($EditClaimData->old_value))->emp_mobile ?? '';
	$NewContact      = optional(json_decode($EditClaimData->new_value))->emp_mobile ?? '';
				...
				<blockquote class="bq1" style="overflow:auto">
					<div class="container">
						<div class="row plr">
							...
							<div class="div12 mbtable">
								<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Contact No. Update -  Request Form</div></div></div>
									...
									<div class="row"> 
										...
										<div class="form-step active"> 
											<div class="row" align="right">
												@php 
												if($Page == 'REQ_APPLY'){
													$BackUrl = 'change-request.contact-change-request-list'; 
												}else if($Page == 'REQ_PROCESS'){
													$BackUrl = 'change-request.contact-change-request-pending-list'; 
												}else{
													$BackUrl = 'change-request.contact-change-request-list'; 
												}
												@endphp
											...
											<fieldset class="fieldbox">
												<legend class="fieldbox-legend"> Existing Contact No.</legend>
												<div class="fieldbox-div">
													<div class="div2 label">
														Existing Contact No. <span class="reqindi">*</span>
													</div>
													<div class="div2">
														<input name="txt_contact_no_old" id="txt_contact_no_old" rows="4" class="tboxsmclass" value="@if(isset($ExistingContact)){{$ExistingContact}}@elseif(isset($ContactNo)){{$ContactNo}}@endif" readonly/>
													</div>
													<div class="row smclearrow"></div>
													<div class="row smclearrow"></div>
												</div>
											...
											<fieldset class="fieldbox">
												<legend class="fieldbox-legend"> New Contact No. </legend>
												<div class="fieldbox-div">
													<div class="div2 label">
														New Contact No. <span class="reqindi">*</span>
													</div>
													<div class="div2">
														<input name="txt_contact_no_new" id="txt_contact_no_new" rows="4" class="tboxsmclass alphanumeric" value="@if(isset($NewContact)){{$NewContact}}@endif"/>
													</div>
													<!-- <div class="div2 label">Attach the Supporting Document</div>
													<input type="file" id="file_emp_contact" name="file_emp_contact" class="step-btn"  accept="image/*"></button> -->
					...
					<div class="row">
						<div class="div12" align="center">
							...
							<input type="hidden" name="wf_module_code" id="wf_module_code" value="{{ encrypt('CONTACT') }}" />
                ...
                if (data != '') {
                    ...
                    if ((EmpData != '') && (EmpData != null)) {
                        ...
                        $.each(EmpData, function (index, element) {
                           ...
                           $("#txt_contact_no_old").val(element.emp_mobile);
                        });
                    }else{
...
var KillEvent = 0;
	$("body").on("click","#btn_save", function(event){
		if(KillEvent == 0){
			var NewMobileNo   	= $("#txt_contact_no_new").val();
			
			if(NewMobileNo == ""){
				BootstrapDialog.alert("New Mobile No should not be empty..!!");
				...
				BootstrapDialog.confirm({
					...
					message: 'Are you sure want to Employee Contact Update ?',
