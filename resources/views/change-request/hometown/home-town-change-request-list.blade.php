...
$Page = $data['Page'] ?? NULL;
//dd($Page);
if(isset($data['HomeTownData']))
{
	
	$HomeTownData = $data['HomeTownData'];
	$HomeTownCount = Count($HomeTownData);
}
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Home Town Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'change-request.home-town-change-request'; @endphp 
									@php $BackUrl =  'request-updates.home-town-update'; @endphp 
									@if($HomeTownCount>0)
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['HomeTownData']))
											@foreach($data['HomeTownData'] as $HomeTownData)
												@php
													$ExistingHomeTown = optional(json_decode($HomeTownData->old_value))->emp_hometown ?? '';
													$NewHomeTown      = optional(json_decode($HomeTownData->new_value))->emp_hometown ?? '';
												@endphp
												<tr data-name="{{ $HomeTownData->emp_no }}" data-status="{{ $HomeTownData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $HomeTownData->emp_no }}</td>
													<td>{{ $HomeTownData->emp_name_payslip }}</td>
													<td>{{ $HomeTownData->designation_name }}</td>
													<td>Home Town Update Request</td>
													<td>{{ $HomeTownData->created_by }}</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('change-request.home-town-change-request',['id'=>encrypt($HomeTownData->change_req_id),'Page'=>encrypt('REQ_APPLY')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
														@endif
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('change-request.hometown-change-request-process',['Page'=>encrypt($Page),'Application'=>encrypt($HomeTownData->change_req_id),'action'=>encrypt('REQ_PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
													</td>
													<td align="center">
														@if($Page == 'REQ_APPLY')
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $HomeTownData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
									</tbody>
