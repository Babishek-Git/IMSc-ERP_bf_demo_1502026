					...
					<div class="container" align="center">
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Designation List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'DesignationMaster.DesignationMaster'; @endphp 
									@php $BackUrl = 'bank.ViewBankMaster'; @endphp 
									<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ NEW DESIGNATION</button>
								</div>
							</div>
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									<thead>
										...
										<th class="rm-sortable" data-col="name"><div class="rm-th-inner">Designation Name<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:12px;height:12px"><path d="M7 8l5-5 5 5M7 16l5 5 5-5"/></svg></div></th>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['DesignationData']))
											@foreach($data['DesignationData'] as $DesignationData)
												<tr data-name="{{ $DesignationData->designation_name }}" data-status="{{ $DesignationData->active == 1 ? 'active' : 'inactive' }}">
													<td>{{ $loop->iteration }}</td>
													<td>{{ $DesignationData->designation_name }}</td>
													<td align="center">
														<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onClick="window.location='{{route('bank.Bank', ['id' => encrypt($DesignationData->id)])}}'" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
													</td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $DesignationData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
									</tbody>
