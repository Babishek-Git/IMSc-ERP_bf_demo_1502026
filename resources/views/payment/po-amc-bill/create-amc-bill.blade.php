...
@php 
if(isset($PoPaymentData)){
    $PaymentId      = collect($PoPaymentData)->pluck('payment_id')->first();
    $BillAmount     = collect($PoPaymentData)->pluck('gross_amount')->first();
    $PoNo           = collect($PoPaymentData)->pluck('amc_file_name')->first();
    $WorkName       = collect($PoPaymentData)->pluck('equip_desc')->first();
    $WorkOrderDate  = collect($PoPaymentData)->pluck('work_order_date')->first();
    $WorkOrderCost  = collect($PoPaymentData)->pluck('grand_total')->first();
    $WorkDuration   = collect($PoPaymentData)->pluck('work_duration')->first();
    $WorkStartDate  = collect($PoPaymentData)->pluck('work_commence_date')->first();
    $WorkEndDate    = '';//collect($PoPaymentData)->pluck('date_of_completion')->first();
    $VendorName     = collect($PoPaymentData)->pluck('name_contractor')->first();
    $VendorAddress  = collect($PoPaymentData)->pluck('addr_contractor')->first();
    $VendorPanNo    = collect($PoPaymentData)->pluck('pan_no')->first();
    $VendorId    = collect($PoPaymentData)->pluck('contid')->first(); 
}
if(isset($VendorBankData)){
    $VendorBankAccNo        = collect($VendorBankData)->pluck('bank_acc_no')->first();
    $VendorBankId           = collect($VendorBankData)->pluck('bank_id')->first();
    $VendorBankBranchId     = collect($VendorBankData)->pluck('branch_id')->first();
    $VendorBankIfsc         = collect($VendorBankData)->pluck('ifsc_code')->first();
    $VendorBankBranchAddr   = collect($VendorBankData)->pluck('branch_addr1')->first();
    $VendorBankName         = collect($VendorBankData)->pluck('bank_name')->first();
}
if(isset($VendorGstData)){
    $VendorGstNo        = collect($VendorGstData)->pluck('gst_no')->first();
}
...
if(isset($PoAmcBudgetData)){
    //$LedgerId               = $PoAmcBudgetData['LedgerId'] ?? NULL;
    //$LedgerName             = $PoAmcBudgetData['LedgerName'] ?? NULL;
    //$LedgerGroupId          = $PoAmcBudgetData['LedgerGroupId'] ?? NULL;
    //$LedgerGroupName        = $PoAmcBudgetData['LedgerGroupName'] ?? NULL;
    $ObjectHeadId           = $PoAmcBudgetData['ObjectHeadId'] ?? NULL;
    $ObjectHeadSubCataId    = $PoAmcBudgetData['ObjectHeadSubCataId'] ?? NULL;
    $ProjectId              = $PoAmcBudgetData['ProjectId'] ?? NULL;
    $GiaId                  = $PoAmcBudgetData['GiaId'] ?? NULL;
    $GiaName                = $PoAmcBudgetData['GiaName'] ?? NULL;
    $ObjectHeadName         = $PoAmcBudgetData['ObjectHeadName'] ?? NULL;
    $ObjectHeadSubCataName  = $PoAmcBudgetData['ObjectHeadSubCataName'] ?? NULL;
    $BudgetSanctionedAmt    = $PoAmcBudgetData['BudgetSanctionedAmt'] ?? NULL;
    $BudgetClaimAmount      = $PoAmcBudgetData['BudgetClaimAmount'] ?? NULL;
    $BudgetReceivedAmount   = $PoAmcBudgetData['BudgetReceivedAmount'] ?? NULL;
    $UptoDtExpenditureAmt   = $PoAmcBudgetData['UptoDtExpenditureAmt'] ?? NULL;
    //$ObjectHeadLedgerMapId  = $PoAmcBudgetData['ObjectHeadLedgerMapId'] ?? NULL;
}
$OptionStr = '';
...
$BillProcessNo = "IMSc/BILL/AMC/XXXX/".$CurrentFinYear;
@endphp
                            ...
                            <div class="div12 mbtable">
                                ...
                                <div class="row">
                                    <div class="div12" style="margin-top:0px;">
                                        <div class="row divhead" align="center">AMC Bill Payment</div>
                                    </div>
                                </div>
                                ...
                                <div class="row innerdiv">
                                    <div class="row" align="right">
                                        ...
                                        @php 
                                        $BackUrl = "payment.indent-bill-payment-creation-list";
                                        @endphp
                                    ...
                                    <div class="row">
                                        <div class="form-step active">
                                            ...
                                            <fieldset class="fieldbox">
                                                <legend class="fieldbox-legend" style="margin-top:0px">AMC Information</legend>
                                                <div class="fieldbox-div">

                                                    
                                                    <div class="div2 label">PO. No.</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_po_no" id="txt_po_no" class="tboxsmclass disable" value="{{ isset($PoNo) ?? '' }}" readonly>
                                                        <input type="hidden" name="txt_payment_id" id="txt_payment_id" class="tboxsmclass disable" value="{{ isset($PaymentId) ? encrypt($PaymentId) : '' }}" readonly>
                                                    ...
                                                    <div class="div2 label pd-l-20">PO. Name</div>
                                                    <div class="div6">
                                                        <input type="text" name="txt_po_name" id="txt_po_name" class="tboxsmclass disable" value="{{ $WorkName ?? '' }}" readonly>
                                                    </div>

                                                    <div class="div2 label">PO. Date</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_po_date" id="txt_po_date" class="tboxsmclass disable" value="{{ $WorkOrderDate ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">PO. Cost</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_po_cost" id="txt_po_cost" class="tboxsmclass disable" value="{{ $WorkOrderCost ?? '' }}" readonly>
                                                    </div>
                                                    
                                                    <div class="div2 label pd-l-20">Work Duration</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_work_duration" id="txt_work_duration" class="tboxsmclass disable" value="{{ $WorkDuration ?? '' }}" readonly>
                                                    </div>
                                                    <div class="row smclearrow"></div>

                                                    <div class="div2 label">Work Starting Date</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_work_start_dt" id="txt_work_start_dt" class="tboxsmclass disable" value="{{ $WorkStartDate ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">Work Completion Date</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_work_end_dt" id="txt_work_end_dt" class="tboxsmclass disable" value="{{ $WorkEndDate ?? '' }}" readonly>
                                                    </div>
                                            ...
                                            <fieldset class="fieldbox">
                                                <legend class="fieldbox-legend">Vendor Information</legend>
                                                <div class="fieldbox-div">
                                                    <div class="div2 label">Vendor Name</div>
                                                    <div class="div10">
                                                        <input type="text" name="txt_vendor_name" id="txt_vendor_name" class="tboxsmclass disable" value="{{ $VendorName ?? '' }}" readonly>
                                                        <input type="hidden" name="txt_vendor_id" id="txt_vendor_id" class="tboxsmclass disable" value="{{ $VendorId ?? '' }}" readonly>
                                                    </div>
                                                    ...
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_bank_name" id="txt_vendor_bank_name" class="tboxsmclass disable" value="{{ $VendorBankName ?? '' }}" readonly>
                                                        <input type="hidden" name="txt_vendor_bank_id" id="txt_vendor_bank_id" class="tboxsmclass disable" value="{{ $VendorBankId ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">Branch Name</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_bank_branch_name" id="txt_vendor_bank_branch_name" class="tboxsmclass disable" value="{{ $VendorBankBranchAddr ?? '' }}" readonly>
                                                        <input type="hidden" name="txt_vendor_bank_branch_id" id="txt_vendor_bank_branch_id" class="tboxsmclass disable" value="{{ $VendorBankBranchId ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">IFSC Code</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_bank_ifsc" id="txt_vendor_bank_ifsc" class="tboxsmclass disable" value="{{ $VendorBankIfsc ?? '' }}" readonly>
                                                    </div>
                                                    ...
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_bank_acc_no" id="txt_vendor_bank_acc_no" class="tboxsmclass disable" value="{{ $VendorBankAccNo ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">PAN No.</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_pan_no" id="txt_vendor_pan_no" class="tboxsmclass disable" value="{{ $VendorPanNo ?? '' }}" readonly>
                                                    </div>
                                                    <div class="div2 label pd-l-20">GST No.</div>
                                                    <div class="div2">
                                                        <input type="text" name="txt_vendor_gst_no" id="txt_vendor_gst_no" class="tboxsmclass disable" value="{{ $VendorGstNo ?? '' }}" readonly>
                                                    </div>
                                                    ...
                                                    <table class="attTable">
                                                        ...
                                                        <tbody>
                                                            <tr id="inputRow">
                                                                ...
                                                                <td align="right">
                                                                    <input type="text" class="tboxsmclass" id="txt_upto_expenditure_amt" name="txt_upto_expenditure_amt" value="{{ isset($UptoDtExpenditureAmt) ? $UptoDtExpenditureAmt : '' }}" readonly>
                                                                </td>
                                                                <td>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="text" class="tboxsmclass" id="txt_balance_amt" name="txt_balance_amt" value="{{ $BalanceAmt }}" readonly>
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                                            ...
                                                            <tr id="inputRow">
                                                                ...
                                                                <td align="center">
                                                                    <select name="cmb_deduct_object_head_0" id="cmb_deduct_object_head_0" class="tboxsmclass ChosenInput">
                                                                        <option value=""> ---- Select ----</option>
                                                                        <!-- {!! $OptionStr ?? '' !!} -->
                                                                        @if(isset($DeductionLedger))
																		@foreach($DeductionLedger as $AllDeductionLedger)
                                                                        <option value="{{ $AllDeductionLedger->ledger_id }}" data-ledgergroup="{{ $AllDeductionLedger->ledger_group_id }}" data-type="L">{{ $AllDeductionLedger->ledger_acc_name }}</option>
                                                                        @endforeach
                                                                        @endif
                                                                    </select>
                                                                </td>
			...
			success: function (data) {  
				if(data != ''){  console.log(data);
                    ...
                    /*if (ObjectHeadSubCataId) {
                        $('#cmb_bill_object_head').append('<option value="'+ObjectHeadSubCataId+'" data-mode="OHSC" data-ohid="'+ObjectHeadId+'" data-subcata="'+ObjectHeadSubCataId+'">'+ObjectHeadSubCataName+'</option>');
                    }else{
                        $('#cmb_bill_object_head').append('<option value="'+ObjectHeadId+'" data-mode="OH" data-ohid="'+ObjectHeadId+'" data-subcata="">'+ObjectHeadName+'</option>');
                    }*/
                    //$("#cmb_bill_object_head").chosen();
