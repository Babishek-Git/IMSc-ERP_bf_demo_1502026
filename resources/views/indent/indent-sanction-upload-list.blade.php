					...
					<div class="container" align="center">
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Indent Sanction Document Upload List</span>
											...
											@foreach($data['Indentdata'] as $Indentdata)
											  	@if(in_array($Indentdata->indent_id, $GetProcessData))
													<tr data-name="{{ $Indentdata->emp_no }}" data-status="{{ $Indentdata->active == 1 ? 'active' : 'inactive' }}">
														...
														<td align="center">
															<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('indent.sanction-document-upload', ['ViewId'=>encrypt($Indentdata->indent_id),'page'=>encrypt('PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Upload </button>
														</td>
													</tr>
												@endif
											@endforeach
