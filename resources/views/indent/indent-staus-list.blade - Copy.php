...
if(isset($data['MaxWorkMoveData'])){
	$MaxWorkMoveData = $data['MaxWorkMoveData'];
}else{
	$MaxWorkMoveData = NULL;
}
					...
					<div class="container">
						<div class="row plr">
							<div class="div12 mbtable">
								<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center"> Indent Status List - View </div></div></div>
										...
										<table class="table-bordered dataTable no-footer" align="center" id="dataTable">
											...
											<tbody>
												...
												<tr>
													...
													$SentOnStr = '';													
													if(isset($MaxWorkMoveData)){
														if(isset($MaxWorkMoveData[$Indentdata->globid])){
															$MaxWorkData = $MaxWorkMoveData[$Indentdata->globid];
															if(filled($MaxWorkData)){
																$SentOn = collect($MaxWorkData)->pluck('created_at')->first();
																if($SentOn != NULL){
																	$SentOnStr = \Carbon\Carbon::parse($SentOn)->format('d/m/Y H:i:s A');
																}
															}
														}
													}								
																...
																if(isset($ToRole)){
																	if(isset($RoleDataArr[$ToRole])){
																		...
																		if(isset($Roles['role_name'])){
																			...
																			echo '<span class="blink" style="font-size:12px;"><span style="color:red;">'.$Roles['role_name'].'</span><br>'.$ToEmpNameStr.'</span>';
																		}
																	}
																}
															}
													...
													<td class="col" align="center">
														<button type="button" onclick="window.location='{{ route('indent.indent-status-view', ['page'=>encrypt('SUBMITED'),'id'=>encrypt($Indentdata->indent_id)])}}'" class="btn btn-default tuploadbtn" title="Click here to View" style="cursor: pointer;"><i class="fa fa-hand-o-right"></i> Status</button>
													</td>
												</tr>
												@php
														$Snumber++;
