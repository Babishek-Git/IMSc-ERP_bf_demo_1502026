					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Employees List</span>
								...
								<div class="rm-toolbar-right">
									...
									<div class="rm-icon-btn" title="Print" onClick="window.location='{{route('employee.export-employee-pdf')}}'">
										 <i class="fa fa-file-pdf-o" style="font-size:15px; color:red; font-weight:600;"></i>
									</div>
									@php $AddUrl  = 'employee.createEmployee'; @endphp 
									@php $BackUrl = 'employee.view-employee-list'; @endphp 
									<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ NEW EMPLOYEE</button>
								</div>
							</div>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['Empdata']))
											@foreach($data['Empdata'] as $Empdata)
												<tr data-name="{{ $Empdata->emp_no }}" data-status="{{ $Empdata->active == 1 ? 'active' : 'inactive' }}">
													<td>{{ $loop->iteration }}</td>
													<td>{{ $Empdata->emp_no }}</td>
													<td>{{ $Empdata->emp_name_payslip }}</td>
													<td>{{ $Empdata->designation_name }}</td>
													<td align="center">
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('employee.createEmployee',['id'=>encrypt($Empdata->emp_no)]) }}'"> <i class='fa fa-edit pt2'></i> View & Edit</button>
													</td>
													<td align="center">
