										...
										<table class="table-bordered table1" width="99%" align="center" id="dataTable">
											<thead>
												<tr class="note heading">
													...
													<th class="colhead" style="text-align:center">Group</th>
													<th class="colhead" style="text-align:center">Division</th>
													<th class="colhead" style="text-align:center">Section</th>
													<th class="colhead" style="text-align:center">SubSection</th>
											...
											<tbody>
												@foreach($data['EmpData'] as $Emp)
													<tr>
														...
														<td align="left">
															{{ $Emp->emp_firstname }}
															@if($Emp->emp_middlename)
																{{ ' ' . $Emp->emp_middlename }}
															@endif
															{{ ' ' . $Emp->emp_lastname }}
														</td>
														<td align="center">{{ $Emp->group }}</td>
														<td align="center">{{ $Emp->division_short_name }}</td>
														<td align="center">{{ $Emp->section }}</td>
														<td align="center">{{ $Emp->subsection }}</td>
														<td align="center">{{ $Emp->designation_name }}</td>
														@if($Emp->active == 1)
														...
														@if($Emp->active == 1)
															<td align="center" style="width:50px">
																<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onclick="edit('{{ route('employee.createEmployee', ['id' => encrypt($Emp->emp_no)]) }}')" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
															</td>
														@else
														...
														<td align="center" style="width:50px">
															@if($Emp->active == 1)
																<button type="button" name = "btn_delete" id = "btn_delete" class="btn btn-default tdelbtn Delete" title="Click here to delete" data-id = "{{ encrypt($Emp->emp_no) }}" style="cursor: pointer;"><i class="fa fa-trash-o pt2"></i></button>												
															@else
														...
														<td align="center" style="width:100px">
															@if($Emp->active == 0)
																<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn  UndoDelete" title="Click here to activate" data-id = "({{ encrypt($Emp->emp_no) }})" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>												
															@else
