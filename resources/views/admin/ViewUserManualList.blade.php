			...
			<div class="grid_12">
				...
				<div class="container" align="center">
					...
					<div class="div10 mbtable" align="center">
						<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center"> User Manual - View </div></div></div>
								...
								<table class="table-bordered dataTable table1" width="99%" align="center" id="dataTable">
									<thead>
										<tr class="note heading">
											...
											<th class="colhead" style="text-align:center">File Name</th>
											<th class="colhead" style="text-align:center; width:100px">Download<br>Manual</th>
									...
									<tbody>
									@foreach($data['ShowUserManualData'] as $UManual)
										<tr>
											<td align="center">{{ $loop->iteration }}</td>
											<td align="left">{{ $UManual->usma_desc }}</td>
											<td align="left">{{ $UManual->usma_name }}</td>
											@if($UManual->active == 1)
												<td>
													<button type="button" onclick="window.location='{{ route('admin.UserManualDownload', ['type'=>encrypt('PDF'),'id'=>encrypt($UManual->usma_id)])}}'" class="btn btn-default btnprimary" title="Click Here to Download PDF"><i class="fa fa-arrows-alt pt2"></i> Download </button>
												</td>
											@else
												<td>
													<button type="button"  class="btn btn-default tuploadbtn" style="cursor: pointer; color:red;"><i class="fa fa-file-pdf-o"></i></button>
												</td>
											@endif
											@if(session('WcmsRoleGroupCode') == 'SUPUSER' )
												@if($UManual->active == 1)
													<td align="center" style="width:100px">
														<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onclick="editUnit('{{ route('admin.UserManualUpload', ['id' => encrypt($UManual->usma_id)]) }}')" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
													</td>
												@else
													<td align="center" style="width:100px">
												...
												<!-- <td align="center" style="width:100px">
													@if($UManual->active == 1)
														<button type="button" name = "btn_delete" id = "btn_delete" class="btn btn-default tdelbtn Delete" title="Click here to delete" data-id = "({{ encrypt($UManual->unitid) }})" style="cursor: pointer;"><i class="fa fa-trash-o pt2"></i></button>												
														@else
												...
												<td align="center" style="width:100px">
													@if($UManual->active == 0)
														<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn  UndoDelete" title="Click here to activate" data-id = "({{ encrypt($UManual->unitid) }})" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>												
													@else
					...
					@php 
					if(session('WcmsRoleGroupCode') == 'SUPUSER'){
						$AddUrl = 'admin.UserManualUpload'; 
					}else{
						$AddUrl = 'dashboard.index'; 
					}		
					@endphp
