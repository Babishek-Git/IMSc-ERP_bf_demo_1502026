...
<script type="text/javascript" language="javascript">
	...
	$("body").on("blur",".TaxCalc", function(event){
		...
		if(NpsEmpContribution != 0){
			NpsEmpContribution = Number(NpsEmpContribution).toFixed();
		}
		...
		let HealthEduCessOld = Math.round((AddSurChargeOld + TaxPayableAfterRebateOld) * 0.04);
		let TaxPlusCessSurchargeOld = Math.round(TaxPayableAfterRebateOld + AddSurChargeOld + HealthEduCessOld);
		let TaxAlreadyPaid = $("#txt_tax_paid_master").val();
		if(TaxAlreadyPaid == ''){
			TaxAlreadyPaid = 0;
		}
		let BalanceTaxToBePaidOld = Math.round(TaxPlusCessSurchargeOld - TaxAlreadyPaid);
		...
		let HealthEduCessNew = Math.round((AddSurChargeNew + TaxPayableAfterRebateNew) * 0.04);
		let TaxPlusCessSurchargeNew = Math.round(TaxPayableAfterRebateNew + AddSurChargeNew + HealthEduCessNew);
	
		let BalanceTaxToBePaidNew = Math.round(TaxPlusCessSurchargeNew - TaxAlreadyPaid);
