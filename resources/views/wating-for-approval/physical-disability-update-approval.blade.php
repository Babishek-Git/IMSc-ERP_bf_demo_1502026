								...
								<div class="row innerdiv">
									<div class="row">  -->
										...
										<div class="form-step active">
											<div class="div12">
												<div class="table-box">
													<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Employee Physical Update -  Waiting for Approval List</div></div></div>
															...
															<table class="table-bordered table1" width="99%" align="center" id="dataTable">
																<thead>
																	<tr>
																		...
																		<th  colspan="2" style="text-align:center">Existing Physical Disability Details</th>
																		<th  colspan="2" style="text-align:center">New Physical Disability Details</th>
																		<th  rowspan="2" style="text-align:center">Supporting Document</th>
																	</tr>
																	<tr>
																		<th style="text-align:center">Physical Disability Type</th>
																		<th style="text-align:center">Physical Disability Percentage</th>
																		<th style="text-align:center">Physical Disability Type</th>
																		<th style="text-align:center">Physical Disability Percentage</th>
																	</tr>
																</thead>
																...
																@if(isset($data['PhysicalData']))
																	@foreach($data['PhysicalData'] as $PhysicalData)  
																		@php
																			...
																			if(isset($EmpGroupedData[$PhysicalData->emp_no])){
																				$EmpData 	= $EmpGroupedData[$PhysicalData->emp_no];
																			...
																			$ExistingPhysicalType = optional(json_decode($PhysicalData->old_value))->phy_challange_type ?? '';
																			$ExistingPhysicalPerc = optional(json_decode($PhysicalData->old_value))->phy_challange_perc ?? '';
																			$NewPhysicalType      = optional(json_decode($PhysicalData->new_value))->phy_challange_type ?? '';
																			$NewPhysicalPerc      = optional(json_decode($PhysicalData->new_value))->phy_challange_perc ?? '';
																		@endphp
																		<tr>
																			<td align="center">{{ $loop->iteration }} </td>
																			<td align="left">{{ $PhysicalData->emp_no }}</td>
																			<td align="left">{{ $PhysicalData->emp_name_payslip }}</td>
																			<td align="left">{{ $OfficeName }}</td>
																			<td align="left">{{ $ExistingPhysicalType }}</td>
																			<td align="left">{{ $ExistingPhysicalPerc }}</td>
																			<td align="left">{{ $NewPhysicalType }}</td>
																			<td align="left">{{ $NewPhysicalPerc }}</td>
																		</tr>
																	@endforeach
																@endif
																</tbody>
