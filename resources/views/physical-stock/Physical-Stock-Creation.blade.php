						...
						<div class="container">
							<div class="row ">
								...
								<div class="div6 mbtable">
									<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Physical Stock Creation</div></div></div>
											...
											<div class="div9">                                                
                                                <div class = "GR1">
                                                    <select class="group tboxclass" name="cmb_group[]" id="cmb_group1" data-group = "1">
                                                        ...
                                                        <!-- <option value="NEW">Computer System & Peripherals</option> -->
                                                        @if(isset($data['ShowGrandParent']))
                                                            @foreach($data['ShowGrandParent'] as $List)
                                                                <option value="{{ $List->material_group_id }}">{{ $List->material_group_name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
	...
	$('body').on("change",".group", function(e){ 
		...
		if(groupid != ""){
			/*if(groupid == "NEWL"){
				...
				$.ajax({ 
					type: 'POST', 
					url: '{{ route("physical-stock.materialGroupFind") }}', 
					...
					success: function (data) {  
						if(data != null){
							...
							$.each(data, function(index, element) {
								OptionList += '<option data-id="'+element.material_group_id+'" data-parid="'+element.material_group_parentid+'" value="'+element.material_group_id+'">'+element.material_group_name+'</option>';
							});
