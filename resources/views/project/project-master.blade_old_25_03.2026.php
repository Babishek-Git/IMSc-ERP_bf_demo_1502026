						...
						<div class="container">
							<div class="row ">
								...
								<div class="div6 mbtable">
									<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Project & Sub Project Creation</div></div></div>
											...
											<div class="div9">                                                
                                                <div class = "GR1">
                                                    <select class="group tboxclass" name="cmb_group[]" id="cmb_group1" data-group = "1">
                                                        ...
                                                        <option value="NEW">ADD NEW MAIN PROJECT</option>
                                                        @if(isset($data['ShowGrandParent']))
                                                            @foreach($data['ShowGrandParent'] as $List)
                                                                <option value="{{ $List->project_id }}">{{ $List->project_name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
	...
	$('body').on("change",".group", function(e){ 
		...
		if(groupid != ""){
			/*if(groupid == "NEWL"){
				...
				$.ajax({ 
					type: 'POST', 
					url: '{{ route("Project.ProjectGroupFind") }}', 
					...
					success: function (data) {  
						if(data != null){
							...
							$.each(data, function(index, element) {
								OptionList += '<option data-id="'+element.project_parentid+'" data-parid="'+element.project_parentid+'" value="'+element.project_parentid+'">'+element.project_name+'</option>';
							});
