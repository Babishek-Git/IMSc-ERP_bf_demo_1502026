...
@php
    ...
    $SessionSectionId           = $data['SessionEmpSectionId'] ?? NULL;
@endphp
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">AMC - Work Waiting For Certification List</span>
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['AmcPoIssudeData']))
											@foreach($data['AmcPoIssudeData'] as $AMCPoData)
												@php
													$AMCMatInwardData = collect($data['AMCMaterialInwardMsterData'] ?? [])->firstWhere('amc_po_id', $AMCPoData->amc_po_order_id);
												@endphp
												@if($AMCPoData->amc_po_issued == 'true' && (!$AMCMatInwardData || $AMCMatInwardData->amc_mat_inward_submit != 'true'))
													@if($AMCPoData->discipline_id == $SessionSectionId)
														<tr data-name="{{ $AMCPoData->created_by }}" data-status="{{ $AMCPoData->active == 1 ? 'active' : 'inactive' }}">
															<td></td>
															<td>{{ $loop->iteration }}</td>
															<td>{{ $AMCTypeArray[$AMCPoData->amc_type_id] }}</td>
															<td>{{ $AMCProvdedBaseONArray[$AMCPoData->amc_baseson_id] }}</td>
															<td>{{ $AMCPoData->amc_file_name }}</td>
															<td>{{ $AMCPoData->equip_desc }}</td>
															<td>{{ $VendorData[$AMCPoData->contid] }}</td>
															<td align="center">
																@if(!empty($AMCMatInwardData) && filled($AMCMatInwardData->amc_master_inward_id))
																    <button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('amc-material.amc-material-inward-entry',['EditId'=>encrypt($AMCMatInwardData->amc_master_inward_id),'page'=>encrypt('MAT_INWAD_EDIT')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
																	<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('amc-material.amc-material-inward-entry', ['EditId'=>encrypt($AMCMatInwardData->amc_master_inward_id),'page'=>encrypt('SUBMIT')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
																@else
																    <button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('amc-material.amc-material-inward-entry',['EditId'=>encrypt($AMCPoData->amc_po_order_id),'page'=>encrypt('EDIT')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
																@endif
															</td>
															<td align="center">
																<button type="button" name="btn_del" id="btn_del"  data-page="{{encrypt('DELETE')}}" data-deltid="{{encrypt($AMCPoData->amc_po_order_id)}}" class="btn btn-default tdelbtn tooltip-l Delete" title="Click here to Delete" style="cursor: pointer;"><i class="fa fa-trash-o pt2"></i></button>
																<!-- <button type="button" name="btn_delete" id="btn_delete" class="btn btn-default tdelbtn" title="Click here to delete" value ='Delete' onclick="window.location='{{ route('amc-purchase-order.amc-purchase-order-submission', ['DeleteId'=>encrypt($AMCPoData->amc_po_order_id),'page'=>encrypt('DELETE')]) }}'"style="cursor: pointer; color: grey;"><i class="fa fa-trash-o pt2"></i></button>												 -->
															</td>
															<!-- <td align="center"><label class="rm-toggle"><input type="checkbox" @if($AMCPoData->active == 1) checked @endif><span class="rm-slider"></span></label></td> -->
														</tr>
													@endif
												@endif
											@endforeach
