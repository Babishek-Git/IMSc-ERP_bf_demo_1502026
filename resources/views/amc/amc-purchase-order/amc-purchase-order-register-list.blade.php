					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">AMC Purchase Order Application Register</span>
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['AMCPORegisterData']))
											@foreach($data['AMCPORegisterData'] as $AMCPoRegData)
												<tr data-name="{{ $AMCPoRegData->created_by }}" data-status="{{ $AMCPoRegData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $AMCTypeArray[$AMCPoRegData->amc_type_id] }}</td>
													<td>{{ $AMCProvdedBaseONArray[$AMCPoRegData->amc_baseson_id] }}</td>
													<td>{{ $AMCPoRegData->amc_file_name }}</td>
													<td>{{ $AMCPoRegData->equip_desc }}</td>
													<td>{{ $VendorData[$AMCPoRegData->contid] }}</td>
													<td align="center">
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('amc-purchase-order.amc-purchase-order-register', ['ViewId'=>encrypt($AMCPoRegData->amc_po_order_id),'page'=>encrypt('VIEW')]) }}'"> <i class='fa fa-check'></i> View Details</button>
													</td>
												</tr>
											@endforeach
										@endif
