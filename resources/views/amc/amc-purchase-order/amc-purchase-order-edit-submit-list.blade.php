...
@php
    ...
    $AMCTypeArray          = $data['AMCTypeData'] ?? NULL;
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">AMC Purchase Order Application Waiting List</span>
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['ShowAMCPOData']))
											@foreach($data['ShowAMCPOData'] as $AMCPoData)
												@if($AMCPoData->amc_po_issued !='true')
													<tr data-name="{{ $AMCPoData->created_by }}" data-status="{{ $AMCPoData->active == 1 ? 'active' : 'inactive' }}">
														...
														<td align="center">
															<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('amc-purchase-order.amc-purchase-order-creation',['EditId'=>encrypt($AMCPoData->amc_po_order_id),'page'=>encrypt('EDIT')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
															<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('amc-purchase-order.amc-purchase-order-creation', ['EditId'=>encrypt($AMCPoData->amc_po_order_id),'page'=>encrypt('SUBMISSION_VIEW')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
														</td>
														<td align="center">
