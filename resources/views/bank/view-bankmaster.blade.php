					...
					<div class="container" align="center">
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Bank List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'bank.Bank'; @endphp 
									@php $BackUrl = 'bank.ViewBankMaster'; @endphp 
									<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ NEW BANK</button>
								</div>
							</div>
								...
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['BankDataView']))
											@foreach($data['BankDataView'] as $BankDataView)
												<tr data-name="{{ $BankDataView->bank_name }}" data-status="{{ $BankDataView->active == 1 ? 'active' : 'inactive' }}">
													<td>{{ $loop->iteration }}</td>
													<td>{{ $BankDataView->bank_name }}</td>
													<td align="center">
														<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onClick="window.location='{{route('bank.Bank', ['id' => encrypt($BankDataView->id)])}}'" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
													</td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $BankDataView->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
									</tbody>
