			...
			<div class="grid_12">
				...
				<div class="container" align="center">
					...
					<div class="div10 mbtable" align="center">
						<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center"> Bank List - View </div></div></div>
										...
										<tr>
											...
											@if($bank->active == 1)
												<td align="center" style="width:100px">
													<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onclick="edit('{{ route('bank.bank', ['id' => encrypt($bank->bank_id)]) }}')" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
												</td>
											@else
												<td align="center" style="width:100px">
											...
											<td align="center" style="width:100px">
												@if($bank->active == 1)
													<button type="button" name = "btn_delete" id = "btn_delete" class="btn btn-default tdelbtn Delete" title="Click here to delete"  data-id = "({{ encrypt($bank->bank_id) }})" style="cursor: pointer;"><i class="fa fa-trash-o pt2"></i></button>												
												@else
											...
											<td align="center" style="width:100px">
												@if($bank->active == 0)
													<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn  UndoDelete" title="Click here to activate" data-id = "({{ encrypt($bank->bank_id) }})" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>												
												@else
					...
					<div class="div1 "></div>
						...
						@php $AddUrl = 'bank.BankMaster'; @endphp 
	...
	$(document).ready(function(){
		...
		$("body").on("click",".Delete", function(event){
			...
			BootstrapDialog.confirm({
				...
				message: 'Are you sure you want to Delete the Bank Name?',
				closable: false, 
				draggable: false, 
				...
				callback: function(result) {
					if(result){
						$.ajax({ 
							type: 'POST', 
							url: "{{ route('bank.DeleteBank') }}", 
							data: {'_token': '{{ csrf_token() }}','Id': Id }, 
							success: function (data){ 
								if(data != null){
									var message = "Bank Name Deleted Successfully";
								}else{
									var message = "Bank Name is not deleted. Please try again";
								}
		...
		$("body").on("click",".UndoDelete", function(event){
			...
			BootstrapDialog.confirm({
				title: 'Confirmation Message',
				message: 'Are you sure you want to Activate the Bank Name?',
				closable: false, 
				draggable: false, 
				btnCancelLabel: 'Cancel', 
				btnOKLabel: 'Ok', 
				callback: function(result) {
					if(result){
						$.ajax({ 
							type: 'POST', 
							url: "{{ route('bank.UndoDelete') }}", 
							data: {'_token': '{{ csrf_token() }}','Id': Id ,'Type': Type}, 
							success: function (data){ 
								if(data != null){
									var message = "Bank Name Activated Successfully";
								}else{
									var message = "Bank Name is not activated. Please try again";
								}
								BootstrapDialog.show({
									title: 'Information',
									message: message,
