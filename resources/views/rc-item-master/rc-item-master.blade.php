						...
						<div class="container">
							<div class="row ">
								...
								<div class="div6 mbtable">
									<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Rate Contract Items Entry</div></div></div>
									<div class="card-body padding-1 ChartCard" id="CourseChart">
										<div class="divrowbox innerdiv pt-2">
																	
											<div class="row smclearrow"></div>                                                                                											
											<div class="div3 label">Item Category </div>
                                                    ...
                                                    <select class="group tboxclass" name="cmb_group[]" id="cmb_group1" data-group = "1">
                                                        ...
                                                        @if(isset($data['ShowGrandParent']))
                                                            @foreach($data['ShowGrandParent'] as $List)
                                                                <option value="{{ $List->rc_item_id }}">{{ $List->rc_item_name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
	...
	$('body').on("change",".group", function(e){ 
		...
		if(groupid != ""){
			/*if(groupid == "NEWL"){
				...
				let GrpApp = '<div class="div3 label">Item Name <span class="reqindi">*</span></div><div class="div9"><input type="text" class="tboxclass newgroup"  name="txt_new_group" id="txt_new_group'+level+'" data-group="'+level+'" autocomplete="off" /><input type="hidden" class="tboxclass newgroup"  name="txt_type" id="txt_type'+level+'" data-group="'+level+'" value="C" /></div><div class="row smclearrow"></div>';
				$('.GR'+level).last().after('<div class = "GR'+nextLevel+'"><div class="row smclearrow">'+GrpApp+'<div class = "GR'+nextLevel+'"><div class="row smclearrow"></div><div class="row smclearrow"></div>');
			}else{
				$.ajax({ 
					type: 'POST', 
					url: '{{ route("Item.ItemGroupFind") }}', 
					data: { "_token": "{{ csrf_token() }}", groupid: groupid, level: level }, 
					dataType: 'json',
					success: function (data) {  
						if(data != null){
							var OptionList = '<option value=""> ----- Select ----- </option>';
								OptionList += '<option value="NEW">ADD NEW ITEM CATEGORY '+nextLevel+'</option>';
								//OptionList += '<option value="NEWL">ADD NEW LEDGER</option>';
							$.each(data, function(index, element) {
								OptionList += '<option data-id="'+element.ledger_group_id+'" data-parid="'+element.rc_item_parentid+'" value="'+element.rc_item_id+'">'+element.rc_item_name+'</option>';
							});
							$('.GR'+level).last().after('<div class = "GR'+nextLevel+'"><div class="row smclearrow"><div class="div3 label"> Item Category '+nextLevel+'</div><div class="div9"><select class="group tboxclass" name="cmb_group[]" id="cmb_group'+nextLevel+'" data-group="'+nextLevel+'">'+OptionList+'</select></div><div class = "GR'+nextLevel+'"><div class="row smclearrow"></div>');
							$("#cmb_group"+nextLevel).chosen();
						}else{
							$('.GR'+level).last().after('<div class = "GR'+nextLevel+'"><div class="row smclearrow"><div class="div3 label">New Item Category '+level+'</div><div class="div9"><input type="text" class="tboxclass"  name="new_group[]" id="new_group'+nextLevel+'" data-group="'+nextLevel+'" required autocomplete="off"/></div><div class = "GR'+nextLevel+'"><div class="row smclearrow"></div>');
						}
					}
				});
			}
