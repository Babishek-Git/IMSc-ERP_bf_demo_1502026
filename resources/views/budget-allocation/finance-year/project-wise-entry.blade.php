                          ...
                          @if($ApplicableTo == 'PROJECT')
                            ...
                            @if(isset($ParentProjectData) && filled($ParentProjectData))
                              @foreach($ParentProjectData as $ParentProject)
                                @php 
                                $Sno = 1; 
                                $ProjectGrantparentId = $ParentProject->project_id; 
                                ...
                                <div class="lom-table-wrap">
                                  <table class="lom-tbl">
                                    ...
                                    <thead>
                                        ...
                                        <th colspan="4">
                                          <div class="row">
                                            <div class="div4 no-margin pd-lr-1">
                                              ...
                                              <input type="hidden" name="txt_apex_project_id[]" id="txt_apex_project_id_{{ $ProjectGrantparentId }}" class="tboxsmclass" value="{{ $ProjectGrantparentId }}" readonly>
                                            </div>
                                            <div class="div2 no-margin pd-lr-1">
                                              <div class="lboxlabel c-case">Sanction Date</div>
                                              <input type="text" name="txt_sanction_date[]" id="txt_sanction_date_{{ $ProjectGrantparentId }}" class="tboxsmclass datepicker" value="{{ isset($ApexSanctionDate) ? Helper::DisplayDateFormat($ApexSanctionDate) : '' }}" readonly>
                                            </div>
                                            <div class="div4 no-margin pd-lr-1">
                                              <div class="lboxlabel c-case">Sanction Amount ( &#8377; )</div>
                                              <input type="number" name="txt_sanction_amount[]" id="txt_sanction_amount_{{ $ProjectGrantparentId }}" class="tboxsmclass" value="{{ isset($ApexSanctionAmt) ? $ApexSanctionAmt : '' }}" readonly>
                                            </div>
                                            <div class="div2 no-margin pd-lr-1">
                                              <div class="lboxlabel c-case">Financial Year</div>
                                              <select type="text" name="cmb_finance_year[]" id="cmb_finance_year_{{ $ProjectGrantparentId }}" class="tboxsmclass ChosenInput">
                                                @php
                                                if(isset($FinYear)){
                                                  echo '<option value="'.$FinYear.'">'.$FinYear.'</option>';
                                                }else{
                                                  $FinYear = '';
                                                }
                                                @endphp
                                      ...
                                      <tr>
                                        <th>S.No.</th><th>Object Head</th><th style="width:250px">Over All Sanction Amount ( &#8377; )</th><th style="width:250px">Sanction Amount (F.Y.) ( &#8377; )</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                          ...
                                          @if($IsMapped == 1)
                                            @if(($SubCataDataCount > 0) && ($IsSubCataApplicable == true))
                                              ...
                                              @foreach($SubCataData as $ObjectHeadSubCata)
                                                @php
                                                  ...
                                                  $ObjectHeadSanctionAmtFy = '';
                                                  if(isset($ApexObjectHeadSanctionDataFy)){
                                                    $ObjeadHeadSancDataFy = $ApexObjectHeadSanctionDataFy
                                                      ->where('fin_year', $FinYear)
                                                      ->where('gia_id', $Gia->gia_id)
                                                      ->where('object_head_id', $ObjectHead->object_head_id)
                                                      ->where('object_head_sub_cata_id', $ObjectHeadSubCata->oh_sub_cata_id)
                                                      ->where('apex_project_id', $ParentProject->project_id);
                                                    if(filled($ObjeadHeadSancDataFy)){
                                                      $ObjectHeadSanctionAmtFy = collect($ObjeadHeadSancDataFy)->pluck('oh_fy_sanctioned_amount')->first();
                                                    }
                                                  }
                                                ...
                                                <tr class="lom-tr-sub">
                                                  ...
                                                  <td class="lom-td-led">
                                                    <input type="number" class="tboxsmclass SanctionAmount" name="txt_oh_sanction_amount_fy[]" id="txt_oh_sanction_amount_fy_{{ $Index }}" value="{{ isset($ObjectHeadSanctionAmtFy) ? $ObjectHeadSanctionAmtFy : '' }}">
                                                  </td>
                                                </tr>
                                                @php $i++; @endphp
                                              @endforeach
                                            @else
                                              ...
                                              <tr>
                                                ...
                                                <td class="lom-td-led">
                                                    <input type="number" class="tboxsmclass SanctionAmount" name="txt_oh_sanction_amount_fy_{{ $ProjectGrantparentId }}[]" id="txt_oh_sanction_amount_fy_{{ $Index }}" value="{{ isset($ObjectHeadSanctionAmtFy) ? $ObjectHeadSanctionAmtFy : '' }}">
                                                </td>
                                              </tr>
                                            @endif
                                            @php $Sno++; @endphp
