								...
								<div class="div8 mbtable">
									...
									<div class="card-body padding-1 ChartCard" id="CourseChart">
										...
										<div class="divrowbox innerdiv pt-2">
											...
											<div class = "div4">
												<input type="text" class="tboxsmclass datepicker" name="rece_date" id="rece_date"  value="">
											</div>
...
$(document).ready(function () {
	...
	function loadSanctionTable(GiaId,Giacode,ProjectId,ProjectParId,FinancialYear,ClaimText,ClaimType) {
		...
		$.ajax({
			type: 'POST',
			url: '{{ route("budget.sanction-claim-details") }}',
			data: {
				"_token": "{{ csrf_token() }}",
				GiaId: GiaId,
				Giacode: Giacode,
						...
						if (AllObectHead.length > 0) {
							...
							if(element){
								if (mappelement.object_head_id == element.object_head_id) {
									...
									var ClaimedId              = ClaimDataByAlloc ? ClaimDataByAlloc.budget_claimed_id : '';
									var RecivedDataByCaim      = GetRecivedData.find(item => item.budget_claimed_id == ClaimedId);
									var RecivedAmount          = RecivedDataByCaim ? RecivedDataByCaim.received_amount : '';
									var RecivedData            = RecivedDataByCaim ? RecivedDataByCaim.received_date : '';
									$("#rece_date").val(RecivedData);
									if(mappelement.is_sup_cata_applicable == true){
										...
										if (AllObjHeadSubCatGroupByData[element.object_head_id] !== undefined) {
											...
											if (ObjectHeadSubCata && ObjectHeadSubCata.length > 0) {
												...
												$.each(ObjectHeadSubCata, function (key, val) {
													...
													SancationTable += '<td><input type="text" class="tboxsmclass decimalnum" readonly style="text-align:right;" name="claim_amount[]" value ="' + ClaimedAmount + '"></td>';
													SancationTable += '<td><input type="text" class="tboxsmclass decimalnum" style="text-align:right;" name="recived_amount[]" value ="' + RecivedAmount + '"></td>';
													SancationTable += '<input type="hidden" name ="bud_allocted_id[]" id="bud_allocted_id"class="tboxsmclass" value="' + BugAllocatedId + '">';
													SancationTable += '<input type="hidden" name ="bud_claim_id[]" id="bud_claim_id"class="tboxsmclass" value="' + ClaimedId + '">';
													SancationTable += '</tr>';
												});
											}
										}
										if (IsSubCata == 0) {
											...
											SancationTable += '<td><input type="text" class="tboxsmclass decimalnum" style="text-align:right;" readonly name="sanction_amount[]" value ="' + SanctionedAmtByObjHead + '"></td>';
											SancationTable += '<td><input type="text" class="tboxsmclass decimalnum" style="text-align:right;" readonly name="claim_amount[]" value ="' + ClaimedAmount + '"></td>';
											SancationTable += '<td><input type="text" class="tboxsmclass decimalnum" style="text-align:right;" name="recived_amount[]" value ="' + RecivedAmount + '"></td>';
											SancationTable += '<input type="hidden" name ="bud_allocted_id[]" id="bud_allocted_id"class="tboxsmclass" value="' + BugAllocatedId  + '">';
											SancationTable += '<input type="hidden" name ="bud_claim_id[]" id="bud_claim_id"class="tboxsmclass" value="' + ClaimedId + '">';
											SancationTable += '</tr>';
	...
	$('body').on("click","#btn_save", function(event){ 
		if(KillEvent == 0){
			...
			if(Gaiid == ''){
				...
				BootstrapDialog.alert("Error : Received Date Should not be in empty..!");
				event.preventDefault();
				event.returnValue = false;	
			}else{
				event.preventDefault();
				BootstrapDialog.confirm({
					...
					message: 'Are you sure you want to save the Budget Received Details?',
