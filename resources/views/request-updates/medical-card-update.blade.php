...
@php
if(isset($data['MedicalData']))
{
	$MedicalData = $data['MedicalData'];
	$MedicalData = Count($MedicalData);
}
@endphp
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Medical Card. Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'change-request.medical-card-request'; @endphp 
									@php $BackUrl = 'request-updates.contact-no-update'; @endphp 
									@if($MedicalData>0)
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['MedicalData']))
											@foreach($data['MedicalData'] as $MedicalData)
												<tr data-name="{{ $MedicalData->emp_no }}" data-status="{{ $MedicalData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $MedicalData->emp_no }}</td>
													<td>{{ $MedicalData->emp_name_payslip }}</td>
													<td>{{ $MedicalData->designation_name }}</td>
													<td>Medicalcard Update Request</td>
													<td>{{ $MedicalData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.id-card-request',['id'=>encrypt($MedicalData->change_req_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View Details</button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $MedicalData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
