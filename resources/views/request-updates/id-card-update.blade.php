					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Id Card. Update Request List</span>
								...
								<div class="rm-toolbar-right">
									@foreach($data['IdData'] as $IdCardData)
									<div class="rm-icon-btn" title="Print" onClick="window.location='{{ route('change-request.export-id-card-pdf',['id'=>encrypt($IdCardData->emp_no)]) }}'">
									...
									@php $AddUrl  = 'change-request.id-card-request'; @endphp 
									@php $BackUrl = 'request-updates.contact-no-update'; @endphp 
									@if($IdData>0)
									    <button type="button" class="rm-new-emp-btn readonly">+ New Request</button>
									@else
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['IdData']))
											@foreach($data['IdData'] as $IdData)
												<tr data-name="{{ $IdData->emp_no }}" data-status="{{ $IdData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $IdData->emp_no }}</td>
													<td>{{ $IdData->emp_name_payslip }}</td>
													<td>{{ $IdData->designation_name }}</td>
													<td>Idcard Update Request</td>
													<td>{{ $IdData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.id-card-request',['id'=>encrypt($IdData->change_req_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View Details</button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $IdData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
