...
@php
if(isset($data['ceaData']))
{
	$ceaData = $data['ceaData'];
	$ceaCount = Count($ceaData);
}
@endphp
					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Reimbursement of CEA Update Request List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl  = 'cea.cea-Application'; @endphp 
							...
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										@if(isset($data['ceaData']))
											@foreach($data['ceaData'] as $ceaData)
												<tr data-name="{{ $ceaData->emp_no }}" data-status="{{ $ceaData->active == 1 ? 'active' : 'inactive' }}">
													<td></td>
													<td>{{ $loop->iteration }}</td>
													<td>{{ $ceaData->emp_no }}</td>
													<td>{{ $ceaData->emp_name_payslip }}</td>
													<td>{{ $ceaData->designation_name }}</td>
													<td>Reimbursement of CEA Update Request</td>
													<td>{{ $ceaData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('cea.cea-Application',['id'=>encrypt($ceaData->reimbursement_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $ceaData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
