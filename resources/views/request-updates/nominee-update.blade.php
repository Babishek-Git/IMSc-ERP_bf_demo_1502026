										...
										@if(isset($data['NomineeData']))
											@foreach($data['NomineeData'] as $NomineeData)
												<tr data-name="{{ $NomineeData->emp_no }}" data-status="{{ $NomineeData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td>Nominee Update Request</td>
													<td>{{ $NomineeData->created_by }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.nominee-update-request',['id'=>encrypt($NomineeData->change_req_id), 'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit </button></td>
													<td align="center">
