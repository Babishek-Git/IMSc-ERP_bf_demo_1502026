										...
										@if(isset($data['FamilyData']))
											@foreach($data['FamilyData'] as $FamilyData)
												<tr data-name="{{ $FamilyData->emp_no }}" data-status="{{ $FamilyData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.empfamilydetails-change-request',['id'=>encrypt($FamilyData->change_req_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit </button></td>
													<td align="center">
