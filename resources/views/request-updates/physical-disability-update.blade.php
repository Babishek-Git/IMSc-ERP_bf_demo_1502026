										...
										@if(isset($data['PhysicalData']))
											@foreach($data['PhysicalData'] as $PhysicalData)
												<tr data-name="{{ $PhysicalData->emp_no }}" data-status="{{ $PhysicalData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('change-request.physicaldisability-change-request',['id'=>encrypt($PhysicalData->change_req_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View Details</button></td>
													<td align="center">
