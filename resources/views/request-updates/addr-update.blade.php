					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl = 'change-request.empaddr-change-request'; @endphp 
									@php $BackUrl = 'request-updates.addr-update'; @endphp 
									@if($Page == 'REQ_APPLY')
									@if($AddressCount>0)
										...
										@if(isset($data['AddressData']))
											@foreach($data['AddressData'] as $AddressData)
												...
												<tr data-name="{{ $AddressData->emp_no }}" data-status="{{ $AddressData->active == 1 ? 'active' : 'inactive' }}">
													...
													<td align="center">
														@if($AddressData->created_by == session('WcmsEmpNo'))
														<button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value="Edit" onclick="window.location='{{ route('change-request.empaddr-change-request',['id'=>encrypt($AddressData->change_req_id),'Page'=>encrypt('REQ')]) }}'"> <i class='fa fa-edit'></i> View & Edit</button>
														@endif
														<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('change-request.empaddr-request',['action'=>encrypt('PROCESS'),'Application'=>encrypt($AddressData->change_req_id),'Page'=>encrypt('VIEW')]) }}'"> <i class='fa fa-check'></i> View & Submit</button>
													</td>
													<td align="center">
														@if($AddressData->created_by == session('WcmsEmpNo'))
