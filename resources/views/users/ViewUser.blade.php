					...
					<div class="container" align="center">
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">User List</span>
								...
								<div class="rm-toolbar-right">
									...
									<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ NEW USER</button>
								</div>
							</div>
							<div class="rm-table-wrap">
								<table id="rm-empTable">
									<thead>
										...
										<th class="rm-sortable" data-col="name"><div class="rm-th-inner">User Name <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:12px;height:12px"><path d="M7 8l5-5 5 5M7 16l5 5 5-5"/></svg></div></th>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['UserListData']))
											@foreach($data['UserListData'] as $UserListData)
												<tr data-name="{{ $UserListData->role_name }}" data-status="{{ $UserListData->active == 1 ? 'active' : 'inactive' }}">
													<td>{{ $loop->iteration }}</td>
													<td>{{ $UserListData->emp_name_payslip }}</td>
													<td>{{ $UserListData->designation_name }}</td>
													<td>{{ $UserListData->role_name }}</td>
													<!-- @if($UserListData->is_portal == 'Y')
														<td align="left">YES</td>
													@else
														<td align="left">NO</td>
													@endif -->
													<td align="center">
														<button type="button" name="btn_edit" id="btn_edit" class="btn btn-default teditbtn estEdit" onClick="window.location='{{route('user.UserCreation', ['id' => encrypt($UserListData->id)])}}'" title="Click here to Edit" style="cursor: pointer;"><i class="fa fa-edit pt2"></i></button>
													</td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $UserListData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
									</tbody>
