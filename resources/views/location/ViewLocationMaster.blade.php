					...
					<div class="container" align="center">
					 	...
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Location View List</span>
								...
								<div class="rm-toolbar-right">
									...
									@php $AddUrl = 'location.location-master'; @endphp 
									@php $BackUrl = 'location.location-master'; @endphp 
									<button type="button" class="rm-new-emp-btn" onClick="window.location='{{route($AddUrl)}}'">+ NEW Location</button>
								</div>
							</div>
									...
									<tbody id="rm-tableBody">
										@if(isset($data['LocationData']))
											@foreach($data['LocationData'] as $LocationData)
												<tr data-name="{{ $LocationData->location_name }}" data-status="{{ $LocationData->active == 1 ? 'active' : 'inactive' }}">
													<td>{{ $loop->iteration }}</td>
													<td>{{ $LocationData->location_name }}</td>
													<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('location.location-master',['id'=>encrypt($LocationData->location_id)]) }}'"> <i class='fa fa-edit'></i> Edit </button></td>
													<td align="center">
													...
													<td align="center"><label class="rm-toggle"><input type="checkbox" {{ $LocationData->active == 1 ? 'checked' : '' }}><span class="rm-slider"></span></label></td>
												</tr>
											@endforeach
										@endif
