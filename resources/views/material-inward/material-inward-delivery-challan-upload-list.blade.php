					...
					<div class="container" align="center">
					 	<div class="div12 no-margin">
							<div class="rm-toolbar">
								<span class="rm-with-selected-btn" id="rm-withSelectedBtn">Material Inward Delivery Challan Upload List</span>
								...
								<table id="rm-empTable">
									...
									<tbody id="rm-tableBody">
										...
										@if(isset($data['showPurchaseOredrData']))
											@foreach($data['showPurchaseOredrData'] as $poSubmitData)
												@if($poSubmitData->mat_cert_sect_id == session('WcmsEmpSec'))
													<tr data-status="{{ $poSubmitData->active == 1 ? 'active' : 'inactive' }}">
														<td></td>
														<td>{{ $Sno}}</td>
														<td>{{ $poSubmitData->work_order_no }}</td>
														<td>{{ $poSubmitData->work_name }}</td>
														<td>{{ Helper::DisplayDateFormat($poSubmitData->work_order_date) }}</td>
														<td>{{ $VendorArr [$poSubmitData->contid] }}</td>
														<td align="center">
															<button type="button" name="btn_submit" class="xlbtndownload m-btm1" id="btn_submit" value="Submit" onclick="window.location='{{ route('material.material-inward-delivery-Challan-upload', ['EditId'=>encrypt($poSubmitData->work_order_id)]) }}'"> <i class='fa fa-check'></i> View & Upload</button>
														</td>
													</tr>
													@php $Sno++;@endphp
												@endif
