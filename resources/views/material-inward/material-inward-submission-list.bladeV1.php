...
$showMaterialInwardData = $data['showMaterialInwardData'] ?? [];
$VendorArr = [];
...
foreach($showMaterialInwardData as $order){
    $WorkId             = $order['sheet_id'];
	$MaterialInwardData = $showPurchaseOredrData->where('work_order_id', $WorkId);
}
						...
						<div class="row plr">
							<div class="div12 mbtable">
										...
										<div class="form-step active">
											<div class="div12">
												<div class="table-box">
													<div class="row"><div class="div12" style="margin-top:0px;"><div class="row divhead" align="center">Material Inward Submission List</div></div></div>
													<div class="card-body padding-1 ChartCard" id="CourseChart">
														<div class="divrowbox innerdiv pt-2">
															...
															<table class="table-bordered table1" width="99%" align="center" id="dataTable">
																...
																<tbody>
																	<tr>
																		@if(isset($showMaterialInwardData))
																			@foreach($showMaterialInwardData as $order)
																			    <?php
																					$WorkId              = $order['sheet_id'];
																			        $InwardId            = $order['master_inward_id'];
																					$MaterialInwardData  = $showPurchaseOredrData->where('work_order_id', $WorkId);
																				?>
																				@foreach($MaterialInwardData as $MatInwardData)
																					<tr>
																						<td align="center">{{ $loop->iteration }} </td>
																						<td align="left">{{ $MatInwardData->work_order_no}}</td>
																						<td align="left">{{ $MatInwardData->work_name }}</td>
																						<td align="left">{{ $VendorArr [$MatInwardData->contid] }}</td>
																						<td align="left">{{ Helper::DisplayDateFormat($MatInwardData->work_order_date) }}</td>
																						<td align="center"><button type="button" name="btn_edit" class="btn btn-default tuploadbtn" id="btn_edit" value=" Edit" onclick="window.location='{{ route('material.material-inward-submission', ['Id' => encrypt($InwardId), 'page' => encrypt('PROCESS')]) }}'"> <i class='fa fa-check'></i> View & Submit </button>
																					</tr>
																				@endforeach
																			@endforeach	
																		@endif
