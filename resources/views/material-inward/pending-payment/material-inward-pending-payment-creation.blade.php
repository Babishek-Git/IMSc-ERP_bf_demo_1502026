...
$BackUrl          = 'material.material-inward-pending-payment';
							...
							<div class="row plr">
													...
													<table class="formtable" align="center" id="RelationshipTable" width="100%">
														<thead> 
															 <tr>
                                                                ...
                                                                <th width="5%">Previous Payment Paid<br>(%)</th>
                                                                ...
                                                                @if(isset($data['MaterialInwardDetailData']))
                                                                    @foreach($data['MaterialInwardDetailData'] as $MatInwardData)
                                                                        @php
                                                                            ...
                                                                            $PreviousRecivedQty  = '';
                                                                            $RecivedQty          = '';
                                                                            if($MatInwardData->po_quantity == $MatInwardData->received_qty){
                                                                                $RecivedQtyText = 'readonly';
                                                                                $RecivedQty     = $MatInwardData->received_qty;
                                                                            }else{
                                                                                $PreviousRecivedQty = $MatInwardData->received_qty;
                                                                            }
                                                                        @endphp
                                                                        <tr>
                                                                            ...
                                                                            <td><textarea name="txt_item_desc[]" id="txt_item_desc_{{$Index}}"  class="tboxsmclass" data-index ='{{$Index}}' data-index ='{{$Index}}'style="width:100%;" readonly value ='{{ $MatInwardData->item_description}}' >{{ $MatInwardData->item_description}}</textarea></td>
                                                                            ...
                                                                            <td><input type="text" name="txt_prev_recd_qty[]" id="txt_prev_recd_qty_{{$Index}}" data-index ='{{$Index}}' class="tboxsmclass" style="width:100%;" readonly value ='{{ $PreviousRecivedQty ?? ""}}'></td>
                                                                            <td><input type="text" name="txt_recd_now_qty[]" id="txt_recd_now_qty_{{$Index}}" data-index ='{{$Index}}' class="tboxsmclass  receivedqty" {{$RecivedQtyText}} style="width:100%;" value ='{{ $RecivedQty}}'></td>
                                                                            ...
                                                                            <td><input type="text" name="txt_previous_pay_perc[]" id="txt_previous_pay_perc_{{$Index}}" data-index ='{{$Index}}' class="tboxsmclass decimalnum" readonly style="width:100%; text-align:right"  value="{{ $MatInwardData->acc_payment_perc ? $MatInwardData->acc_payment_perc : '' }}" ></td>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td colspan="2" align="right">Total Pay (Rs.)</td>
                                ...
                                <div class="row">
                                    <div class="div12" align="center">
                                        ...
                                        <input type="hidden" name="txt_from_Page" id="txt_from_Page" value='{{$FromPage}}' />
                                    </div>		
                                </div>  
                            </div>                         
						</div>
...
$('document').ready(function(){
    ...
    $('body').on('change', '.percvalue', function(event) {
        var Index         = $(this).data('index');
        var TotalCost     = Number($('#txt_total_cost_'+ Index).val()) || 0;
        var PayPerc       = Number($('#txt_item_pay_perc_'+ Index).val()) || 0;
        var PrevPayPerc   = Number($('#txt_previous_pay_perc_'+ Index).val()) || 0;
        var TotalPerc     = PayPerc + PrevPayPerc;
        if (TotalPerc > 100) {
            var AllowedPerc = 100 - PrevPayPerc;
            BootstrapDialog.alert('Previous payment already completed ' + PrevPayPerc + '%. Only ' + AllowedPerc + '% can be entered.');
			event.returnValue = false;
            $('#txt_item_pay_perc_' + Index).val('');
        }else{
