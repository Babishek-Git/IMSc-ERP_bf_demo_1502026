...
{   
    public function __construct(){ 
        ...
        $this->EmpPdfList = new EmpPdfList(); 
        $this->FellowshipCategoryName = new FellowshipCategoryName(); 
    }
    ...
    {   
        ...
        if(isset($request->id)){
            ...
            $Editpaydetail          = $this->EmployeePayLevel->showEmployeeCurrentPayLevel($EmpNo);
        ...
        if($IsRegisterForm == 1){ 
            ...
            if($Page == 'EDIT_REG'){
                $OfficeGroupId          = $EditEmpBasicData->pluck('group_id')->first(); 
                $OfficeDivisionId       = $EditEmpBasicData->pluck('division_id')->first(); 
                $OfficeGroupIdList      = [$OfficeGroupId];
                $OfficeDivisionIdList   = [$OfficeDivisionId];
                $OfficeDivisonList      = $this->Office->ShowMultipleOfficeWithRepToOffice($OfficeGroupIdList,'D');
                $OfficeSectionList      = $this->Office->ShowMultipleOfficeWithRepToOffice($OfficeDivisionIdList,'S'); 
            }else{ 
                $OfficeGroupIdList  = collect($officeList)->pluck('office_id')->toArray();
                $OfficeDivisonList  = $this->Office->ShowMultipleOfficeWithRepToOffice($OfficeGroupIdList,'D');
                $OfficeSectionList  = [];
            }
            return view('employee.CreateEmployee')->with('data',compact('officeList','OfficeDivisonList','OfficeSectionList','desiginationList','showGrandParent',
            ...
            'EditPliEmpInsuranceData','GuideLists','Page','PdfLists','Editpaydetail'));  
        }  
    ...
    public function SaveEmpBasicDetails($request){ 
        ...
        $EmpFirstName      = $request->txt_emp_first_name;
        if($EmpFirstName == ''){
            $EmpFirstName = NULL;
        }
        // if($EmpMiddleName == ''){
        ...
        $EmpIntercom          = $request->txt_office_mobno;
        $EmpMobile            = $request->txt_perm_mobno;
        ...
        $PdfrNpdf             = $request->cmb_pdfrnpdf;
        if ($PdfrNpdf == 'Others') {
            $PdfrNpdf = $request->txt_other_pdf_name;
            DB::table('erp_emp_pdflist')->insert([
                'emp_pdf_name' => $PdfrNpdf,
                'active'       => 1,
                'created_at'   => now(),
                'updated_at'   => now()
            ]);
        ...
        try {
           ...
           // $SaveData['emp_last_name']                  = $EmpLastName;
            $SaveData['emp_first_name']                  = $EmpFirstName;
            ...
            $SaveData['emp_personal_mobile_no']         = $EmpMobile; 
            ...
            $SaveData['emp_pdf_name']                   = $PdfrNpdf; 
            //dd($SaveData);
            ...
            if($request->filled('txt_pay_amount')){ 
                $PayAmount     = $request->txt_pay_amount;
                $this->EmployeePayLevel->deleteEmployeePayLevel($EmpIcno);
                $SavePayData['emp_no']          = $EmpIcno;
                $SavePayData['basic_salary']    = $PayAmount;
                $SavePayData['is_current']         = true;
                $SavePayData['active']             = 1;
                $SavePayData['created_at']         = NOW();
                $SavePayData['created_by']         =session('WcmsEmpNo'); 
                $SaveEmpPayLevel = $this->EmployeePayLevel->createEmployeeCurrentPayLevel($SavePayData);
                if($request->filled('txt_pay_component')){
                    $this->EmployeePayComponent->deleteEmployeePayComonent($EmpIcno);
                    $PayComponent     = $request->txt_pay_component;
                    $SaveData['emp_no']             = $EmpIcno;
                    $SaveData['component_id']       = $PayComponent;
    ...
    {   
        ...
        $menuCodes          = $UIConfigs->pluck('menu_module_code')->toArray();
        $selectedPay        = $this->EmployeePayComponent->where('emp_no', $RegEmpData->emp_no)->where('is_current', true)->pluck('component_id')->toArray();
        $componentFilterArr = array("DEDU","EARN");
        $payComponents = PayComponent::withType()->active()
            ->whereHas('componentType', function ($q) use ($componentFilterArr) {
                $q->whereIn('component_type_code', $componentFilterArr);
