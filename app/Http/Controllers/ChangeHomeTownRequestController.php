...
class ChangeHomeTownRequestController extends Controller
{   
    ...
    public function EmpChangeHomeTownReqSelfService(Request $request)
    {  
        $EditClaimData = NULL;
        $Page = "REQ_APPLY";
        $message = NULL;
        ...
        if(isset($request->btn_save)){ 
            ...
            if(filled($ErrArr))
            {
                ...
                return redirect()->route('change-request.hometown-change-request-list');
            }
            DB::beginTransaction();
            try { 
        ...
        return view('change-request.hometown.home-town-change-request')->with('data',compact('Empdata','EditClaimData','Page'));
    } 

    public function EmpChangeHomeTownReqSelfServiceList(){
        $Page        = 'REQ_APPLY'; 
        $EmpNo       = session('WcmsEmpNo'); 
        $HomeTownData = $this->ChangeRequest->ShowEmpPendingChangeRequest(NULL,$EmpNo,'HOMETOWN'); 
        return view('change-request.hometown.home-town-change-request-list')->with('data', compact('HomeTownData','Page'));
    }
    public function EmpChangeHomeTownReqPendingList(){
        $Page        = 'REQ_PROCESS'; 
        $EmpNo       = session('WcmsEmpNo'); 
        $HomeTownData = $this->ChangeRequest->ShowEmpPendingChangeRequest(NULL,NULL,'HOMETOWN');
        return view('change-request.hometown.home-town-change-request-list')->with('data', compact('HomeTownData','Page'));
    }
    public function EmpChangeHomeTownReqProcess(Request $request)
    {  
        if(isset($request->SubmitApplication)){
            try {
                ...
                Session::put('ALertMesage', $message);
                if($Page == "REQ_PROCESS"){
                    return redirect()->route('change-request.hometown-change-request-pending-list');
                }else{
                    return redirect()->route('change-request.hometown-change-request-list');
                }
            }
        ...
        if(($IsCompleted == NULL)||($IsCompleted == false)){
            if(($TargetRoles == '')||($TargetRoles == NULL)){
                ...
                $WorkFlowActionData = $this->WorkFlowService->CheckForwardAndBackward('HOMETOWN',$ApplicationId,$TargetRoles,$ApprAuthRole);
               
            }
        }
        //dd($WorkFlowActionData);
        return view('change-request.hometown.home-town-change-request-view')->with('data',compact('ApplicationId','Action','Empdata','EditClaimData','Page','WorkFlowActionData'));  
