...
class ChangeMaritalRequestController extends Controller
{   
    ...
    public function EmpChangeMaritalStatusReqSelfService(Request $request)
    {  
        $EditClaimData = NULL;
        $Page = "REQ_APPLY";
        $message = NULL;
        ...
        if(isset($request->btn_save)){  
            ...
            $EmpSpouseIncome   = $request->txt_spouse_income;
            $EmpSpouseAadhar   = $request->txt_spouse_aadhar;
            $EmpSpouseBloodGrp = $request->txt_blood_group;
            $ChangeRequestId   = $request->hid_change_id;

            $rules = [
				'EmpSpouseName'     => 'required|max:50',
				'EmpSpouseDoB'      => 'required|max:20',
                'EmpSpouseIncome'   => 'required|max:10',
				'EmpSpouseAadhar'   => 'required|max:15',
                'EmpSpouseBloodGrp' => 'required|max:3',
			];

			$ValidateData = [
                'EmpSpouseName'      => $EmpSpouseName,
				'EmpSpouseDoB'       => $EmpSpouseDoB,
                'EmpSpouseIncome'    => $EmpSpouseIncome,
				'EmpSpouseAadhar'    => $EmpSpouseAadhar,
                'EmpSpouseBloodGrp'  => $EmpSpouseBloodGrp,
			];
            ...
            if($Validate->fails())
             {
                ...
                foreach ($ValidateFields as $ValidFieldName => $ValidRules) 
                {
                    ...
                    if($EmpSpouseIncome == "EmpSpouseIncome"){
                        //$ItemNo = '';
                        $ErrArr[] = "Error : Invalid Employee Spouse Income.";
                    }
                    if($EmpSpouseAadhar == "EmpSpouseAadhar"){
                        //$ItemDesc = '';
                        $ErrArr[] = "Error : Invalid Employee Employee Aadhar.";
                    }
                }
            }
            if(filled($ErrArr))
            {
                ...
                return redirect()->route('change-request.marital-change-request-list');
            }
            ...
            try { 
                ...
                $SaveArr1['Spouse_income']   = $request->txt_spouse_income;
                $SaveArr1['Spouse_aadhar']   = $request->txt_spouse_aadhar;
                $SaveArr1['Spouse_bloodgrp'] = $request->txt_blood_group;
                $SaveData = json_encode($SaveArr1);

                //$SaveArr2['emp_address'] = $request->txt_cont_address;
                //$SaveData = json_encode($SaveArr1);

                $SaveArr['module_code'] =  'MARRIAGE';
        ...
        return view('change-request.maritalstatus.maritalstatus-change-request')->with('data',compact('Empdata','EditClaimData','Page'));
    } 

    public function EmpChangeMaritalStatusReqSelfServiceList(){
        $Page        = 'REQ_APPLY'; 
        $EmpNo       = session('WcmsEmpNo'); 
        $MaritalData = $this->ChangeRequest->ShowEmpPendingChangeRequest(NULL,$EmpNo,'MARRIAGE');
        return view('change-request.maritalstatus.martial-status-change-request-list')->with('data', compact('MaritalData','Page'));
    }
    public function EmpChangeMaritalStatusReqPendingList(){
        $Page        = 'REQ_PROCESS'; 
        $EmpNo       = session('WcmsEmpNo'); 
        $MaritalData = $this->ChangeRequest->ShowEmpPendingChangeRequest(NULL,NULL,'MARRIAGE'); 
       // dd($MaritalData);
        return view('change-request.maritalstatus.martial-status-change-request-list')->with('data', compact('MaritalData','Page'));
    }
    public function EmpMaritalStatusReqProcess(Request $request)
    { 
        if(isset($request->SubmitApplication)){
            try { //dd($request);
                ...
                Session::put('ALertMesage', $message);
                if($Page == "REQ_PROCESS"){
                    return redirect()->route('change-request.marital-change-request-pending-list');
                }else{
                    return redirect()->route('change-request.marital-change-request-list');
                }
            }
        ...
        if(($IsCompleted == NULL)||($IsCompleted == false)){
            if(($TargetRoles == '')||($TargetRoles == NULL)){
                ...
                $WorkFlowActionData = $this->WorkFlowService->CheckForwardAndBackward('MARRIAGE',$ApplicationId,$TargetRoles,$ApprAuthRole);
            }
        }   //dd($WorkFlowActionData);
        return view('change-request.maritalstatus.maritalstatus-change-request-view')->with('data',compact('ApplicationId','Action','Empdata','EditClaimData','Page','WorkFlowActionData'));  
